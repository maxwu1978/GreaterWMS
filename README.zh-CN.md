...
Pack List 流程。Pack List 页面现在是只读页面，Pack List 和 QC 结果统一
通过 CLI/AI Agent 导入。CLI 使用网页端相同的 `token` 请求头，通过应用
API 操作，不直接操作数据库。
...
`--dry-run`/`--confirm`。Pack List 导入、确认、替换和 QC 导入统一通过
CLI/AI Agent 执行，页面只显示结果。Pack List 删除和批量清理不支持。
