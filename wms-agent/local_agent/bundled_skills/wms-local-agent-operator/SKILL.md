...
operational fields. Billing, nested client settings, SKU attributes, provider
secrets, and allowed-tool governance previews remain preview-only. Platform
user-management writes are available through the governed `tools/wms.mjs` CLI,
not through a free-form local-agent chat confirmation; use `wms users create`,
`wms users update`, or `wms users reset-password` with the required evidence
and strong-confirmation flags.
