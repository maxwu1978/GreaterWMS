 ...
 *   可选:--days 0,1(一进程连跑)| --cleanup(收尾清理)| --api <URL>
...
const API = arg('api', 'https://api.maxsmartwms.online');
const HTTP_TIMEOUT_MS = Math.max(1000, Number(arg('timeout-ms', 30000)) || 30000);
const MAX_RETRIES = Math.max(0, Math.min(8, Number(arg('max-retries', 3)) || 3));
const USER = arg('user'); const PASS = arg('pass');
if (!USER || !PASS) {
  console.log('用法: node wms-sim-node.mjs --user <SIM账号> --pass <密码> [--register] [--day 0 | --days 0,1] [--cleanup] [--api URL] [--timeout-ms 30000] [--max-retries 3]');
  process.exit(1);
}
// ---- 浏览器环境垫片 ----
const LS = new Map();
...
window.SIM = (() => {
  ...
  async function api(path, { method = 'GET', body, token, operator, agent, headers, retry = 0 } = {}) {
    ...
    try {
      ...
      return [0, { detail: 'network:' + e.message, retry_limit: MAX_RETRIES }];
    }
  ...
  // ---------- 通用小步骤 ----------
  const freeSlot = async flow => { const slots = await freeSlots(flow, 1); return slots[0] || null; };
  const freeSlots = async (flow, quantity) => {
    const [, s] = await api('/staging/slots/?flow=' + flow);
    return (Array.isArray(s) ? s : [])
      .filter(x => (x.status || '').toUpperCase() !== 'ACTIVE' && !x.occupied && (x.available !== false))
      .slice(0, quantity)
      .map(x => x.bin_name || x.name)
      .filter(Boolean);
  };
  ...
  async function inboundAsnSN({ agent = true } = {}) { // AI-agent 导入 pack list 的 SN 柜
    ...
    if (slots.length) step(c, 'reserve-staging', ...(await api(`/asn/reserve-staging/${aid}/`, { method: 'POST', token: S.staff.WH.token, operator: S.staff.WH.id, body: { staging_bins: slots } })));
    await boardCheck('WH', asn, { present: true, note: '到货后仓库应见指派卸货指示' });
    const drv = 'SIM-Leo';
    step(c, 'unload-start', ...(await api(`/asn/preload/${aid}/`, { method: 'POST', token: S.staff.WH.token, operator: S.staff.WH.id, body: { unload_driver: drv, staging_bins: slots } })));
  ...
  async function outboundCommon(c, { goods, qty, sn = false, serials = [], agent = false }) {
    ...
    const createPayload = {
      ...
      ship_to: 'SIM-CUST-A Dock',
    };
    let createResult;
    if (agent) {
      createResult = await agentExec('/dn/list/', createPayload, 'outbound.create', tokOB);
    } else {
      createResult = await api('/dn/list/', Object.assign({ method: 'POST', body: createPayload }, tokOB));
    }
    ...
    let detailResult;
    if (agent) {
      detailResult = await agentExec('/dn/detail/', detailBody, 'outbound.detail.create', Object.assign({ resourceId: dn }, tokOB));
    } else {
      detailResult = await api('/dn/detail/', Object.assign({ method: 'POST', body: detailBody }, tokOB));
    }
  ...
  async function outboundFlow(kind, { agent = false } = {}) { // clean|sn|podexc|cancelreturn
    ...
    const tno = 'TR-' + r.dn;
    // Dispatch atomically moves an outbound transport order to IN_TRANSIT;
    // do not submit a second transition, just verify the resulting state.
  ...
  async function verify() {
    ...
    for (const g of S.skus) {
      ...
      const checks = [
        // goods_qty includes open ASN reservations; onhand/can_order remain physical.
    ...
    if (act.length) {
      S.ledger.notes.push('active staging on closed refs=' + JSON.stringify(act.map(a => ({ flow: a.flow, reference_code: a.reference_code, bin_name: a.bin_name }))));
    }
...
SIM._cfg.api = API;
await SIM.seed();
if (!SIM._state.seeded) { console.error('seed 未完成,退出'); process.exit(1); }
const daysArg = arg('days', arg('day', '0'));
for (const d of String(daysArg).split(',').map(s => s.trim()).filter(Boolean)) {
  await SIM.runDay(Number(d));
}
await SIM.roles();
await SIM.verify();
SIM.report();
if (has('cleanup')) await SIM.cleanup();
