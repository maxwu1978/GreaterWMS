#!/usr/bin/env python3
"""Verify package-origin rollups and summaries against production."""
...
class ProductionVerifier:
    ...
    def run(self) -> dict[str, Any]:
        ...
        order_number = f"INB-PKGORG-{suffix}"
        tracking_prebooked = f"TRK-PKGORG-{suffix}-1"
        carton_dock = f"CTN-PKGORG-{suffix}-2"
        ...
        inbound = self.request(
            ...
            json={
                ...
                "reference_number": f"REF-PKGORG-{suffix}",
                "lines": [{"line_number": 31, "sku_id": sku["id"], "quantity": 5}],
            },
        )
        order_id = inbound["id"]
        ...
        receipt = self.request(
            ...
            json={
                ...
                "package_count": 1,
                "pallet_count": 1,
                "measured_weight_kg": 5.5,
                "receiving_note": "Package origin queue verification",
            },
        )
        ...
        packages = self.request("GET", f"/receiving/inbound/{order_id}/packages")
        packages_by_number = {pkg["package_number"]: pkg for pkg in packages}
        detail_packages_by_number = {pkg["package_number"]: pkg for pkg in detail_after["lines"][0]["packages"]}
        ...
        return {
            ...
            "orders_list_summary": {
                ...
                "packages_stored": listed.get("packages_stored"),
                "packages_needing_action": listed.get("packages_needing_action"),
                "packages_prebooked": listed.get("packages_prebooked"),
                "packages_dock_created": listed.get("packages_dock_created"),
                "internal_labels_total": listed.get("internal_labels_total"),
            ...
            "package_origin_list": {
                ...
                for number, package in packages_by_number.items()
            },
            "package_origin_detail": {
                str(number): {
                    "package_origin": package.get("package_origin"),
                    "status": package.get("status"),
                }
                for number, package in detail_packages_by_number.items()
            },
        }
